...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO plans (name, price, description, features, button_text) VALUES ('$name', '$price', '$description', '$features', '$button_text')";

    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('New plan created successfully'); window.location.href='manage-plans.php';</script>";
    } else {
...
<body class="bg-gray-100 text-gray-800">
    <div class="container mx-auto p-4">
        <h1 class="text-4xl font-bold text-center mb-8">Add New Plan</h1>
        <form method="post" action="" class="bg-white p-8 rounded-lg shadow-lg max-w-lg mx-auto">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="button_text" id="button_text" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
            </div>

            <div class="flex justify-end">
                <input type="submit" value="Add Plan" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">
            </div>
        </form>
